...
const BulkOperationPopup = ({ open, setOpen, setDelete, actionType }) => {
  const cancelButtonRef = useRef(null);

  const getDecodedType = (type) => {
                  ...
                  <div className="sm:flex sm:items-start">
                    ...
                    >
                      {getDecodedType(actionType) === 'delete' ? (
                        ...
                        <NoSymbolIcon className="h-6 w-6 text-red-500" aria-hidden="true" />
                      )}
                    </div>
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900">
                        <span className="capitalize">{getDecodedType(actionType)}</span> records
                      </Dialog.Title>
                      <div className="mt-2">
