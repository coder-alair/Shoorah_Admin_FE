...
const CustomSelect = ({ data, label, disabled, selected, setSelected, isMultiple }) => {
      ...
      {!disabled && (
        ...
        >
          <div
            ...
            } flex justify-between w-full rounded-3xl border border-gray-300 px-4 py-2 text-sm overflow-hidden font-medium text-gray-700 shadow-sm focus:border-shoorah-primary focus:outline-none focus:ring-shoorah-primary`}
          >
            {/* {label} here */}
            {isMultiple ? (
              selected.length > 0 ? (
                <div className="flex items-center gap-2">
                  {selected.map((data) => (
                    <span
                      className="relative flex gap-x-1 whitespace-nowrap bg-shoorah-primary px-1 rounded-full text-white items-center"
                      key={data.value}
                    >
                      {data.name}
