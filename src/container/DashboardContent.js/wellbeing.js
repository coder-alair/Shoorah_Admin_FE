...
const Overallwellbeing = ({ userId }) => {
  ...
  const getJournalData = (action) => {
    Api.getOverallWellJournalData(action).then((response) => {
  ...
  const getShuruData = (action) => {
    Api.getOverallWellShuruData(action).then((response) => {
      if (response?.data?.meta?.code === 1) {
        let data = response.data.data;
        let result = [];
        data.map((value) => {
  ...
  const getPersonalData = (action) => {
    Api.getOverallWellPersonalData(action).then((response) => {
      if (response?.data?.meta?.code === 1) {
        let data = response.data.data;
        let result = [];
        data.map((value) => {
  ...
  const getProfessionalData = (action) => {
    Api.getOverallWellProfessionalData(action).then((response) => {
      if (response?.data?.meta?.code === 1) {
        let data = response.data.data;
        let result = [];
        data.map((value) => {
  ...
  return (
    <div>
      <div className="dark:bg-shoorah-darkBgTabColor py-[1rem] px-5 dark:text-white bg-white rounded-[10px]">
        <p className="text-sm mb-[2rem] text-center font-[400] text-gray-400">
          System data sets for all users, starting the Shoorah Journey Vs Where the user is at now.
        ...
        <div className="mt-3 flex flex-col  overflow-auto">
          ...
          <div className=" dark:border-shoorah-darkBgColor my-[2rem] border-gray-200 w-full">
            <p className="text-center text-2xl ">Overall Journal wellbeing</p>
          ...
          <div className=" dark:border-shoorah-darkBgColor my-[2rem] border-gray-200 w-full">
            <p className="text-center text-2xl ">Overall Shuru wellbeing</p>
            <LineChartContainer data={shuruData} />
          </div>
          <div className=" dark:border-shoorah-darkBgColor my-[2rem] border-gray-200 w-full">
            <p className="text-center text-2xl ">Overall Personal Emotions wellbeing</p>
            <LineChartContainer data={personalEmotionData} />
          </div>
          <div className=" dark:border-shoorah-darkBgColor my-[2rem] border-gray-200 w-full">
            <p className="text-center text-2xl ">Overall Professional Emotions wellbeing</p>
