...
const pages = [{ name: 'Sleep', href: '/content-management/sounds' }];
...
const Sounds = (props) => {
  ...
  const handlePagination = (pageNumber, pageSize, searchKey, sortBy, sortOrder) => {
    ...
    if (isDraftsOpen) {
      Api.getDraftSoundsList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then(
        (response) => {
          if (response?.data?.meta?.code === 1) {
            setCurrentPage(pageNumber);
            setSoundsList(response?.data?.data);
    ...
    Api.getSoundsList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then((response) => {
      if (response?.data?.meta?.code === 1) {
        setCurrentPage(pageNumber);
        setSoundsList(response?.data?.data);
        setTotalCount(response?.data?.meta?.totalRecords);
  ...
  const deleteHandler = (id) => {
    setLoader(true);
    setSearchTerm('');
    Api.deleteSound(id).then((response) => {
      if (response?.data?.meta?.code === 1) {
        handlePagination(1, PAGE_SIZE, '', sortBy, sortOrder);
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      isEdit: '/content-management/sounds/add-edit',
      isView: '/content-management/sounds/view',
      isDelete: true
    }
  ];
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Sleep | Shoorah Admin</title>
      </Helmet>
      {loader ? <Loader /> : null}
      ...
      <div className="mt-6 px-3">
          ...
          <div className="flex">
            ...
            <div className="self-center">
              <SearchInput
                ...
                placeholder="Search by sleep or expert name"
              />
            </div>
          </div>
          <div className="mt-3 flex items-center justify-between ">
            ...
            <div className="flex flex-wrap gap-y-2 ">
              ...
              <div className="ml-3 self-center">
                <button
                  onClick={() => navigate('/content-management/sounds/add-edit')}
                  type="button"
                  className="inline-flex items-center justify-center rounded-3xl border border-transparent bg-gradient-to-r from-shoorah-primary to-shoorah-secondary px-4 py-2 text-sm font-medium text-white shadow-sm hover:shoorah-primary focus:outline-none focus:ring-2 focus:ring-shoorah-primary focus:ring-offset-2 sm:w-auto"
                >
                  Add New
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              <div>
                <SelectMenu
                  ...
                  label="Status"
                  isRequired={false}
                  setSelectedMenu={(data) => filterHandler(data, 'soundStatus')}
                />
              </div>
              <div>
                <SelectMenu
        ...
        <div className="mt-4">
          <Table
            ...
            contentType={4}
            addNewURL="/content-management/sounds/add-edit"
