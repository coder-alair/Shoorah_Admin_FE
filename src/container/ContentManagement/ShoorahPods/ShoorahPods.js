...
const pages = [{ name: 'Shoorah Pods', href: '/content-management/shoorah-pods' }];
...
const PAGE_SIZE = 10;
const ShoorahPods = (props) => {
  const navigate = useNavigate();
  const wrapperRef = useRef(null);
  const [loader, setLoader] = useState(true);
  const [shoorahPodsList, setShoorahPodsList] = useState([]);
  ...
  const handlePagination = (pageNumber, pageSize, searchKey, sortBy, sortOrder) => {
    ...
    if (isDraftsOpen) {
      Api.getDraftShoorahPodsList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then(
        (response) => {
          if (response?.data?.meta?.code === 1) {
            setCurrentPage(pageNumber);
            setShoorahPodsList(response?.data?.data);
    ...
    Api.getShoorahPodsList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then((response) => {
      if (response?.data?.meta?.code === 1) {
        setCurrentPage(pageNumber);
        setShoorahPodsList(response?.data?.data);
        setTotalCount(response?.data?.meta?.totalRecords);
  ...
  const deleteHandler = (id) => {
    setLoader(true);
    setSearchTerm('');
    Api.deleteShoorahPods(id).then((response) => {
      if (response?.data?.meta?.code === 1) {
        handlePagination(1, PAGE_SIZE, '', sortBy, sortOrder);
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      isView: '/content-management/shoorah-pods/view',
      isEdit: '/content-management/shoorah-pods/add-edit',
      isDelete: true
    }
  ];
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Shoorah Pods | Shoorah Admin</title>
      </Helmet>
      {loader ? <Loader /> : null}
      ...
      <div className="mt-4 px-3">
          ...
          <div className="flex">
            ...
            <div className="self-center">
              <SearchInput
                ...
                placeholder="Search by pod or expert name"
              />
            </div>
          </div>
          <div className="mt-3 flex justify-between items-center">
            ...
            <div className="flex flex-wrap gap-y-2">
              ...
              <div className="ml-3 self-center">
                <button
                  onClick={() => navigate('/content-management/shoorah-pods/add-edit')}
                  type="button"
                  className="inline-flex items-center justify-center rounded-3xl border border-transparent bg-gradient-to-r from-shoorah-primary to-shoorah-secondary px-4 py-2 text-sm font-medium text-white shadow-sm hover:shoorah-primary focus:outline-none focus:ring-2 focus:ring-shoorah-primary focus:ring-offset-2 sm:w-auto"
                >
                  Add New
        ...
        >
          <div
            ref={wrapperRef}
            className="absolute p-5 right-5 z-[2] mt-2 w-[100%] sm:w-[600px] dark:bg-shoorah-darkBgColor  dark:text-white lg:w-[700px] mx-auto origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none  "
          >
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              <div>
                <SelectMenu
                  ...
                  label="Status"
                  isRequired={false}
                  setSelectedMenu={(data) => filterHandler(data, 'podStatus')}
                />
              </div>
              <div>
                <SelectMenu
        ...
        <div className="mt-4">
          <Table
            columns={columns}
            data={shoorahPodsList}
            name={'shoorah_pods_table'}
            setDeleteId={deleteHandler}
            bottomBorder={totalCount > PAGE_SIZE}
            setSortBy={(sort) => handleSortBy(sort)}
            contentType={5}
            addNewURL="/content-management/shoorah-pods/add-edit"
      ...
      <div>
        {shoorahPodsList.length > 0 && !loader ? (
          <Pagination
