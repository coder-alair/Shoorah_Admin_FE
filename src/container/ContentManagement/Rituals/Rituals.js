...
const pages = [{ name: 'Rituals', href: '/content-management/rituals' }];
...
const Rituals = (props) => {
  ...
  const handlePagination = (pageNumber, pageSize, searchKey, sortBy, sortOrder) => {
    ...
    if (isDraftsOpen) {
      Api.getDraftRitualList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then(
        (response) => {
    ...
    Api.getRitualList(pageNumber, pageSize, searchKey, sortBy, sortOrder).then((response) => {
      if (response?.data?.meta?.code === 1) {
        setCurrentPage(pageNumber);
        setRitualsList(response?.data?.data);
        setTotalCount(response?.data?.meta?.totalRecords);
  ...
  const deleteHandler = (id) => {
    setLoader(true);
    setSearchTerm('');
    Api.deleteRitual(id).then((response) => {
      if (response?.data?.meta?.code === 1) {
        handlePagination(1, PAGE_SIZE, '', sortBy, sortOrder);
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      isEdit: '/content-management/rituals/add-edit',
      isView: '/content-management/rituals/view',
      isDelete: true
    }
  ];
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Rituals | Shoorah Admin</title>
      </Helmet>
      {loader ? <Loader /> : null}
      ...
      <div className="mt-6 px-3">
          ...
          <div className="mt-3 flex justify-between items-center">
            ...
            <div className="flex flex-wrap gap-y-2">
              ...
              <div className="ml-3 self-center">
                <button
                  onClick={() => navigate('/content-management/rituals/add-edit')}
                  type="button"
                  className="inline-flex items-center justify-center rounded-3xl border border-transparent bg-gradient-to-r from-shoorah-primary to-shoorah-secondary px-4 py-2 text-sm font-medium text-white shadow-sm hover:shoorah-primary focus:outline-none focus:ring-2 focus:ring-shoorah-primary focus:ring-offset-2 sm:w-auto"
                >
                  Add New
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              <div>
                <SelectMenu
                  ...
                  label="Status"
                  isRequired={false}
                  setSelectedMenu={(data) => filterHandler(data, 'ritualStatus')}
                />
              </div>
              <div>
                <SelectMenu
        ...
        <div className="mt-4">
          <Table
            ...
            contentType={7}
            addNewURL="/content-management/rituals/add-edit"
