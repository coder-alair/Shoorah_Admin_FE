...
function AddCompany() {
  ...
  const pages = [
    ...
    {
      name: `${company?.action === 'edit' ? 'Edit Company' : 'Add Company'}`,
      href: '/B2B-company-list/addCompany',
      current: true
    }
  ];
  ...
  useEffect(() => {
    if (company?._id) {
      Api.getCompanyData(company?._id).then((res) => {
        if (res.data.meta.code === 1) {
          let price = planList.filter((price) => {
            return price.name.toLowerCase().includes(res.data.data.seat_price);
          });
          ...
          setCompanyData({
            ...
            no_of_seat_bought: res.data.data.no_of_seat_bought,
            seat_price: res.data.data.seat_price,
            seat_active: res.data.data.seat_active,
            contract_start_date: currentDateWithFormat(new Date(res.data.data.contract_start_date)),
            contract_end_date: currentDateWithFormat(new Date(res.data.data.contract_end_date)),
            contract_length: monthDiff(
              new Date(res.data.data.contract_start_date),
              new Date(res.data.data.contract_end_date)
            ),
            contract_progress: res.data.data.contract_progress,
            b2b_interest_via: res.data.data.b2b_interest_via,
            terms_agreed: res.data.data.terms_agreed,
            contract_sent: res.data.data.contract_sent,
            contract_signed: res.data.data.contract_signed,
            invoice_raised: res.data.data.invoice_raised,
            payment_complete: res.data.data.payment_complete,
            restrict_company: res.data.data.restrict_company,
            salesman: res.data.data.salesman,
            plan: res.data.data.plan,
            currency: res.data.data.currency ? res.data.data.currency : 'gbp',
            company_type: res.data.data.company_type,
            other_admin_emails: res.data.data.other_admin_emails?.join(','),
            shuru_usage: res.data.data.shuru_usage,
            peap_usage: res.data.data.peap_usage,
            vat_tax: !!res.data.data.vat_tax,
            discount: res.data.data.discount,
            b2bPlan: res?.data?.data?.b2bPlan,
            product: price[0]?.value,
            auto_renew: res.data.data.auto_renew,
            is_charity: res.data.data.is_charity
          });
          setCurrentPrice(res?.data?.data?.seat_price);
          setProduct(res?.data?.data?.product);
          setContractStartDate(currentDateWithFormat(new Date(res.data.data.contract_start_date)));
          setContractEndDate(currentDateWithFormat(new Date(res.data.data.contract_end_date)));
          setB2BPlan(res?.data?.data?.b2bPlan);
          setTrialDaysLeft(res.data.data.trialDaysLeft);
          setNumber(res.data.data?.contact_number ? res.data.data?.contact_number.toString() : '');
        }
      });
    }
  }, [company?._id]);
      ...
      if (companyData.contract_length !== '-1') {
        setCompanyData({
          ...
          contract_end_date: currentDateWithFormat(
            addMonthOnDate(companyData.contract_start_date, parseInt(value))
          )
        });
        setContractEndDate(
    ...
    if (validate.isValid) {
      ...
      if (company._id) {
        ...
        const data = {
          ...
          trial,
          is_charity: companyData.is_charity,
          trialDays,
          paymentCheck: check,
          product: stripePlan,
        ...
        Api.editCompany(data, company._id)
          .then((response) => {
            setLoader(false);
  ...
  return (
    <div>
      ...
      <div className="mt-4 mx-3 rounded-3xl p-4 bg-white dark:bg-shoorah-darkBgTabColor dark:text-white">
                ...
                <p className="text-lg font-medium">
                  ...
                  {currentPrice ? (
                    <span className="text-sm text-gray-400">
                      ( current seat price is {currentPrice} £{' '}
                      {parseInt(currentPrice) > 10 ? 'which is not valid' : ''} modify per seat
                      price for new price){' '}
                    </span>
                  ) : null}
                </p>
        ...
        {/* Charity/Trial Options  */}
        <Disclosure defaultOpen={true}>
          {({ open }) => (
            <>
              <DisclosureButton className="flex w-full justify-between rounded-lg mt-6">
                <span className="text-lg font-medium">Charity/Trial Options (optional)</span>
                <ChevronUpIcon className={`${!open ? 'rotate-180 transform' : ''} h-5 w-5`} />
              </DisclosureButton>
              <DisclosurePanel className="grid gap-4 lg:grid-cols-4 lg:grid-cols-4 sm:grid-cols-2">
                <div className="flex flex-col justify-between mt-3">
                  <div className="flex flex-row justify-between items-center">
                    <div>Charity Toggle</div>
                    <Switch
                      checked={companyData.is_charity}
                      onChange={() =>
                        setCompanyData(() => ({
                          ...companyData,
                          is_charity: !companyData.is_charity
                      ...
                      className={`${companyData.is_charity ? 'bg-[#3FA349]' : 'bg-shoorah-grayText'}
                                    relative inline-flex h-[30px] w-[58px] shrink-0 
                                    cursor-pointer rounded-full border-2 border-transparent 
                                    transition-colors duration-200 ease-in-out focus:outline-none 
                                    focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`}
                    >
                      <span
                        aria-hidden="true"
                        className={`${companyData.is_charity ? 'translate-x-7' : 'translate-x-0'}
                                        pointer-events-none inline-block h-[26px] w-[26px] transform rounded-full 
                                        bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
                      />
                    </Switch>
                  ...
                  <div className="text-xs text-[#666666]  dark:text-white dark:border-none">
                    When made active this contract will be on charity account type for specified
                    contract length.
                  </div>
                </div>

                {!companyData.is_charity && (
                  <>
                    <div className="flex flex-col justify-between mt-3">
                      <div className="flex flex-row justify-between items-center">
                        <div>
                      ...
                      <div className="text-xs text-[#666666]  dark:text-white dark:border-none">
                        When made active add number of days to make them use dashboard for trial
                        days.
                      </div>
                    </div>
