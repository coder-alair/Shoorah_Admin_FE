...
import { PER_PAGE, STATUS_FOR_DROPDOWN, USER_TYPE, USER_TYPE_B2B } from '../../utils/constants';
...
function SubAdmin() {
  ...
  const handlePagination = (pageNumber, pageSize, searchKey, query, sortBy, sortOrder) => {
    ...
    if (userData.userType === 0) {
      Api.getAdmins(pageNumber, pageSize, searchKey, query, sortBy, sortOrder).then((response) => {
      ...
      CompanyApi.getAdmins(pageNumber, pageSize, searchKey, query, sortBy, sortOrder).then(
        (response) => {
          if (response?.data?.meta?.code === 1) {
            setCurrentPage(pageNumber);
            setAdminList(response?.data?.data);
  ...
  const deleteHandler = (id) => {
    ...
    if (userData.userType === 0) {
      ...
      CompanyApi.deleteAdmin(id).then((response) => {
        if (response?.data?.meta?.code === 1) {
          handlePagination(1, selectedPerPage?.value, '', filterQuery, sortBy, sortOrder);
          successToast(response?.data?.meta?.message);
        } else if (response?.data?.meta?.code === 0) {
  ...
  return (
    <div className="relative">
      <Helmet>
        <meta charSet="utf-8" />
        <title>Sub Admin | Shoorah Admin</title>
      </Helmet>
      {loader ? <Loader /> : null}
      ...
      <div className="mt-6 px-3">
        ...
        >
          <div
            ref={wrapperRef}
            className={`absolute p-6 right-0 z-[2] ${filterCriteria.userType || filterCriteria.accountStatus ? 'mt-[4rem]' : 'mt-2'} w-[100%] sm:w-[600px] lg:w-[700px] mx-auto origin-top-right rounded-md dark:bg-shoorah-darkBgTabColor dark:text-white bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none  `}
          >
