...
import { Api } from '../api';
import { errorToast, isSuperAdmin, isSuperAndSubAdmin, successToast } from '../utils/helper';
...
function ViewNotification() {
  ...
  const handlePagination = (pageNumber, pageSize, searchKey) => {
    ...
    if (isSuperAndSubAdmin()) {
      Api.getAllNotifications(2, pageNumber, pageSize, searchKey).then((response) => {
      ...
      CompanyApi.getAllNotifications(2, pageNumber, pageSize, searchKey).then((response) => {
        if (response?.data?.meta?.code === 1) {
          setCurrentPage(pageNumber);
          setNotificationList(response?.data?.data);
          setTotalCount(response?.data?.meta?.totalNotifications);
  ...
  const handleDeletePopup = () => {
    ...
    if (isSuperAdmin()) {
      Api.deleteNotification(idToDelete, deleteType)
        .then((response) => {
          if (response?.data?.meta?.code === 1) {
      ...
      CompanyApi.deleteNotification(idToDelete, deleteType)
        .then((response) => {
          if (response?.data?.meta?.code === 1) {
            successToast(response?.data?.meta?.message);
            handlePagination(1, PAGE_SIZE, '');
    ...
    <>
      ...
      {notificationList?.length > 0 ? (
        <div className="mt-3">
          ...
          <div className="mt-3 mx-3 overflow-hidden border border-gray-200 bg-white dark:bg-shoorah-darkBgTabColor dark:border-none dark:text-white rounded-t-md">
