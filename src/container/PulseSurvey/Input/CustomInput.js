  ...
  return (
    <div className="container border-shoorah-secondary ">
      <div className="container xl:px-4 md:px-3 md:py-3">
        <div className="space-y-1 mr-14 w-full flex align-middle justify-start">
          ...
          {handlePlus || handleMinus ? (
            <div className="flex flex-nowrap">
              <img src={PlusIcon} loading='lazy' onClick={handlePlus} className="w-10 max-[640px]:w-8" />
              <img src={MinusIcon} loading='lazy' onClick={handleMinus} className="w-10 max-[640px]:w-8" />
            </div>
          ) : (
            ''
          )}
        ...
        {error ? (
          <span className="pl-2 error text-xs text-red-400">Please Enter Question</span>
        ) : (
          ''
        )}
      </div>
